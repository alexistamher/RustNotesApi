...
use crate::util::{ crypto_manager::encrypt_aes_gcm, session_manager::check_session };
use crate::util::crypto_manager::decrypt_aes_gcm;

pub struct CryptoMiddlewareFactory;

impl<S, B> Transform<S, ServiceRequest>
    for CryptoMiddlewareFactory
    where
        ...
        B: MessageBody + 'static
{
    ...
    fn new_transform(&self, service: S) -> Self::Future {
        ready(Ok(CryptoMiddleware { service: std::rc::Rc::new(service) }))
    }
}
    ...
    fn call(&self, mut req: ServiceRequest) -> Self::Future {
        ...
        Box::pin(async move {
            ...
            let session_id = match headers.get("x-session-key").and_then(|v| v.to_str().ok()) {
                ...
                None => {
                    return Err(
                        actix_web::error::ErrorBadRequest("Invalid or missing x-session-key header")
                    );
                }
            };
            ...
            let nonce_encoded = match headers.get("x-nonce").and_then(|v| v.to_str().ok()) {
                ...
                None => {
                    return Err(
                        actix_web::error::ErrorBadRequest("Invalid or missing x-nonce header")
                    );
                }
            };
            ...
            let session = match check_session(session_id) {
                ...
                None => {
                    return Err(actix_web::error::ErrorUnauthorized("Session not found or expired"));
                }
            };
            ...
            let nonce = BASE64.decode(nonce_encoded).map_err(|_|
                actix_web::error::ErrorBadRequest("invalid Base64 for nonce")
            )?;
            let ciphertext = BASE64.decode(body).map_err(|_|
                actix_web::error::ErrorBadRequest("invalid Base64 for ciphertext")
            )?;

            let decrypted_body = decrypt_aes_gcm(&ciphertext, &nonce, &aes_key).map_err(|_|
            ...
            if !res_head.status().is_success() {
                return Ok(ServiceResponse::new(req_part, res_head.set_body(res_body)).map_into_left_body());
            }
            let body_bytes = actix_web::body::to_bytes(res_body).await.map_err(Into::into)?;
            ...
            let (ciphertext, nonce) = encrypt_aes_gcm(
                body_str,
                &aes_key
            ).map_err(|_|
            ...
            let encoded_body_response = BASE64.encode(&ciphertext);
            ...
            new_res.insert_header(("x-nonce", BASE64.encode(&nonce)));
